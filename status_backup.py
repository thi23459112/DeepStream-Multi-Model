...
#  一執行就同時顯示：服務狀態 + 辨識 log + 雲端上傳 log + 地端上傳 log
#  按鍵即時生效（免按 Enter）：
#     1 開啟 = 啟動 deepstream.service + 啟動雲端/地端 timer
#     2 關閉 = 停止 deepstream.service + 停止雲端/地端 timer
#     q 離開
#
#  與舊版差異：
#     - 上傳排程由「crontab 單隻 PutAPI」改為「systemd 兩個 timer」
#       （send-api-cloud.timer / send-api-local.timer）
#     - 新增「地端上傳」log 面板；上傳 log 改讀各自的 .log 檔
# =============================================================
...
LOCAL_TIMER        = "send-api-local.timer"
...
# 兩隻 API 的 log 檔（由 send_interval_api*.py 產生）
CLOUD_LOG = str(PROJECT_DIR / "send_interval_api.live.log")
LOCAL_LOG = str(PROJECT_DIR / "send_interval_api_local.live.log")

# 固定設定
MAX_KEEP        = 500      # 日誌保留行數
API_PANEL_ROWS  = 12       # 每個上傳 log 面板行數
...
console.print(f"[green]✓[/] 地端 timer: [yellow]{LOCAL_TIMER}[/]  log: {LOCAL_LOG}")
console.print("")
...
local_buffer = deque(maxlen=MAX_KEEP)   # 地端上傳日誌
stop_event   = threading.Event()

# 狀態快取
state = {"cloud_timer": None, "local_timer": None}
...
def start_all():
    ...
    subprocess.run(["sudo", "systemctl", "start", LOCAL_TIMER], check=False)
    state["cloud_timer"] = True
    state["local_timer"] = True
...
def stop_all():
    ...
    subprocess.run(["sudo", "systemctl", "stop", LOCAL_TIMER], check=False)
    state["cloud_timer"] = False
    state["local_timer"] = False
...
def render_dashboard(term_height: int) -> Layout:
    ...
    local_text = timer_label(state["local_timer"])

    # ----- 標題面板 -----
    header = Panel(
        f"辨識服務：{service_text}    "
        f"雲端上傳：{cloud_text}    地端上傳：{local_text}",
    ...
    footer = Panel(
        "[green]1[/] 開啟(辨識+雲端+地端)     "
        "[red]2[/] 關閉(辨識+雲端+地端)     [dim]q[/] 離開",
        border_style="dim",
    )

    # ----- 面板高度計算 -----
    # 版面：header(3) + 辨識 + 雲端(API_PANEL_ROWS) + 地端(API_PANEL_ROWS) + footer(3)
    recog_lines = max(3, term_height - 6 - 2 * API_PANEL_ROWS - 2)
    ...
    local_panel = make_log_panel(local_buffer, API_PANEL_ROWS - 2,
                                 "🏠 地端上傳日誌 (send_interval_api_local)", "cyan")
    ...
    layout.split_column(
        ...
        Layout(local_panel, size=API_PANEL_ROWS),
...
def action_open():
    console.print("\n[green]🔓 開啟：啟動辨識服務 + 雲端 timer + 地端 timer...[/]")
    start_all()
    console.print("[green]✅ 完成。[/]")


def action_close():
    console.print("\n[yellow]🔒 關閉：停止辨識服務 + 雲端 timer + 地端 timer...[/]")
    stop_all()
...
def main():
    ...
    threading.Thread(target=make_tail_reader(LOCAL_LOG, local_buffer), daemon=True).start()

    # 初始化讀取 timer 狀態
    console.print("[dim]讀取目前狀態中...[/]")
    try:
        state["cloud_timer"] = timer_is_enabled(CLOUD_TIMER)
        state["local_timer"] = timer_is_enabled(LOCAL_TIMER)
    except Exception:
